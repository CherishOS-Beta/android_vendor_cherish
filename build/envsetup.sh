...
- repopick:        Utility to fetch changes from CherishOS Gerrit.
...
function githubremote()
{
    ...
    local REMOTE=$(git config --get remote.cherish.projectname)
...
function installrecovery()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop org.pixelexperience.device | grep -q "$CHERISH_BUILD");
    then
...
function cherishgerrit() {
    ...
    local user=`git config --get review.gerrit.cherishos.org.username`
    local review=`git config --get remote.cherish.review`
    local project=`git config --get remote.cherish.projectname`
    local remote_branch=udc
    local command=$1
    shift
    case $command in
        help)
...
function cherishrebase() {
    ...
    cd $dir
    repo=$(git config --get remote.cherish.projectname)
    echo "Starting branch..."
    repo start tmprebase .
    echo "Bringing it up to date..."
    repo sync .
    echo "Fetching change..."
    git fetch "http://gerrit.cherishos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
